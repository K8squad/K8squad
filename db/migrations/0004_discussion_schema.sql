-- 0004_discussion_schema.sql — Per-Project Discussion Room schema (Arch §7.5, Story 10.1 / ISI-2702)
--
-- Forward-only, versioned migration (same discipline as 0001_coord_schema.sql). Applied exactly
-- once, in filename order (after 0003_coord_outbox.sql), by the apiserver migration runner on
-- startup. There is NO down migration.
--
-- NUMBERING NOTE: the Story 10.1 design substrate (ISI-2702) names this file `0003_discussion_schema`.
-- Version 0003 was taken on `main` by `0003_coord_outbox.sql` (Story 12.1 / ISI-2260) between the
-- substrate being authored and this build landing, so it is renumbered to 0004 to keep the version
-- sequence monotonic and unique. Content is otherwise the §7.5 canonical shape.
--
...
-- authority and this build removes the old file. Do NOT run both.
