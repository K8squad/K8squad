...
// Vitest runs with the console/ package as root, so resolve from cwd.
const COMPONENT_DIR = join(process.cwd(), "src", "components", "discussion");
...
describe("AC5 — no coordination affordance in the room UI", () => {
  ...
  it("renders no coordination-state control element (button/role) for custody", () => {
    ...
    const re = /<button[^>]*>[^<]*\b(claim|assign|checkout|complete|transition)\b/i;
    const offenders = files.filter((f) => re.test(code(f)));
    expect(offenders).toEqual([]);
  });
});
