...
// (ISI-2915 / ISI-3103). It installs three things once, at process start:
//
...
//
// Both signals export to stdout by default ("stdout exporter is fine for now",
// ISI-3103 AC). The call sites — Tracer(), Extract(), Inject(), and the slog
// logger — never name an exporter, so swapping stdout for OTLP later is a
// one-line change in Setup and nothing else moves.
package telemetry
...
// ShutdownFunc flushes and stops the trace and log pipelines. Call it on
// process exit (with a bounded context) so buffered spans/logs are not lost.
