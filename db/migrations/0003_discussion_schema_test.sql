-- 0003_discussion_schema_test.sql — runnable self-check for the discussion schema (Story 10.1 / ISI-2702)
--
-- No framework, no fixture: plain SQL that fails loudly if the §7.5 invariants break. Runs against a
-- throwaway Postgres AFTER 0003_discussion_schema.sql is applied:
--
--     psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f db/migrations/0003_discussion_schema.sql \
--                                              -f db/migrations/0003_discussion_schema_test.sql
--
