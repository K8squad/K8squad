...
import { AuthorBadge } from "../src/components/discussion/AuthorBadge";
import { deriveAuthorBadge } from "../src/lib/discussion/provenance";
...
describe("<AuthorBadge> — AC2 rendering", () => {
  it("renders an agent name", () => {
    render(<AuthorBadge badge={deriveAuthorBadge({ authorType: "agent", authorName: "planner-1", metadata: null })} />);
  ...
  it("renders a visible 'unattributed' marker for a provenance defect (never blank)", () => {
    render(
      <AuthorBadge
        badge={deriveAuthorBadge({ authorType: "" as unknown as "agent", authorName: "", metadata: null })}
      />,
    );
    const chip = screen.getByTestId("author-chip");
  ...
  it("carries its base colour as --chip-base so the border derives from base (8.9)", () => {
    render(<AuthorBadge badge={deriveAuthorBadge({ authorType: "human", authorName: "u", metadata: null })} />);
    const chip = screen.getByTestId("author-chip");
