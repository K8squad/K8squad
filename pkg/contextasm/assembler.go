...
func (a *Assembler) Assemble(ctx context.Context, req AssembleRequest) (*AssembleResult, error) {
	...
	wi, err := a.sources.WorkItem(ctx, req.Run.Spec.WorkItemRef, pinnedWiRev)
	if err != nil {
		return nil, fmt.Errorf("contextasm: work item %q: %w", req.Run.Spec.WorkItemRef, err)
	}
	meta, err := a.sources.ProjectMeta(ctx, req.Run.Spec.ProjectRef.Name, pinnedGoalRev)
	if err != nil {
		return nil, fmt.Errorf("contextasm: project meta: %w", err)
	}
	recall, err := a.sources.MemoryRecall(ctx, req.TeamID, req.Run.Spec.ProjectRef.Name, pinnedDocIDs, a.TopK)
	if err != nil {
		return nil, fmt.Errorf("contextasm: memory recall: %w", err)
	}
	arts, err := a.sources.Artifacts(ctx, string(req.Run.UID))
	if err != nil {
