...
import { deriveAuthorBadge, extractRun, runHref } from "../src/lib/discussion/provenance";

// AC2 (the crux): badge derivation is exhaustive over the provenance triple.

const base = { authorName: "", metadata: null as Record<string, unknown> | null };

describe("deriveAuthorBadge — AC2 provenance triple", () => {
  it("agent: authorType=agent → agent badge with the agent name", () => {
    const b = deriveAuthorBadge({ authorType: "agent", authorName: "planner-1", metadata: null });
  ...
  it("human: authorType=human → human badge", () => {
    const b = deriveAuthorBadge({ authorType: "human", authorName: "henrik", metadata: null });
  ...
  it("system: authorType=system → system badge", () => {
    const b = deriveAuthorBadge({ authorType: "system", authorName: "", metadata: null });
