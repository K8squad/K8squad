#!/usr/bin/env python3
"""Story X.1 (ISI-2240) — hostile-Run blast-radius containment, a DESIGN-LEVEL MODEL GATE
(arch §4.3, §12, §17.1, S4, NFR-SEC1). Required CI artifact (L4 Security lane).

SCOPE — read this first (retro-review ISI-2537 F2, reconciled via ISI-2539). This harness is an
always-on falsification of the isolation *model*: the two postures below are hand-authored, NOT parsed
from the repo's Kubernetes manifests. It proves the §12/§17.1 blast-radius invariants are internally
sound and that a weakened control is DETECTABLE — it does NOT (yet) prove the manifests actually
shipped in `config/` match the contained posture. That anchoring is deliberately future work:
`config/` currently ships only CRDs + a webhook, so there are no NetworkPolicy/RBAC Team manifests to
bind to. Two follow-ups are tracked on the L4 lane under ISI-2245:
  (1) config-anchoring — derive `posture_contained` from the rendered Team-namespace manifests so a real
      regression (e.g. a ClusterRole granting `secrets: list`, a root-mounted workspace PVC) flips it;
  (2) a runtime kind-based S4 e2e — assert containment against live NetworkPolicy/RBAC in a cluster.
Until (1)/(2) land, this gate makes NO "runtime containment" claim; it is the model falsification.
