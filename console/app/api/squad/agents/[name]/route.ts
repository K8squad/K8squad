// app/api/squad/agents/[name]/route.ts — BFF agent-authoring-detail proxy (ISI-4002, ADR-0016).
//
// GET-ONLY. Proxies the apiserver's per-kind authoring-spec read
// (GET /api/squad/agents/{name}, fleetlist.go) — the compose WRITE wire shape the
// Compose EDIT form hydrates from so opening a agent shows its real spec and a PUT
