...
// (db/migrations 0001 + 0003_coord_outbox + 0005_reconcile_step) — uuid-keyed
// claim rows carrying reconcile_step + fence_token, the §6.5 audit log, and the
// canonical §6.6 outbox.
//
...
//	INSERT INTO coord.outbox    ('run', …, 'reconcile_advanced', …)   — §6.6 exactly-once projection (canonical outbox)
//
...
// fail records the first infrastructure error (sticky). A guard rejection (0 rows,
// live foreign lease) is NOT a failure — those are reported through the calling
// method's boolean, not Err(). Call sites report the failure via their own return
// (false / zero value), so fail captures only; it returns nothing.
func (s *ProdReconcileStore) fail(err error) {
	if s.err == nil {
		s.err = err
	}
}
...
func (s *ProdReconcileStore) Advance(expected, next reconcile.Step, fence *int64) bool {
	...
	// (3) §6.6 outbox row — the exactly-once projection event on the CANONICAL
	//     coord.outbox (0003_coord_outbox), co-committed so a downstream relay
	//     (NATS §17.4; the CRD status/SSE projector in a later story) can never
	//     observe a step advance that has no projection event, nor an event for an
	//     advance that rolled back. It is an `entity='run'` /
	//     `event_type='reconcile_advanced'` domain event; from_step/to_step and the
	//     committed fence ride in the versioned jsonb payload. project_id (the
