...
// Releases are OPT-IN (ISI-3956 S5a): a nil or false toggle must not add
// RecordTypeRelease, and only an explicit true includes it — the inverse of
// the opt-out default the other kinds use.
func TestSnapshotOptionsReleasesOptIn(t *testing.T) {
	r := &Reconciler{}
	...
	// nil Releases (all other kinds default-on): no release type.
	on := r.snapshotOptions(&ksquadapi.RepoSyncSpec{Mirror: &ksquadapi.RepoMirrorSpec{}})
	if has(on.Types, scm.RecordTypeRelease) {
		t.Fatalf("nil Releases must not include release type, got %+v", on.Types)
	}
	// explicit false: still excluded.
	off := r.snapshotOptions(&ksquadapi.RepoSyncSpec{Mirror: &ksquadapi.RepoMirrorSpec{Releases: ptrBool(false)}})
	if has(off.Types, scm.RecordTypeRelease) {
		t.Fatalf("Releases=false must not include release type, got %+v", off.Types)
	}
	// explicit true: included.
	yes := r.snapshotOptions(&ksquadapi.RepoSyncSpec{Mirror: &ksquadapi.RepoMirrorSpec{Releases: ptrBool(true)}})
	if !has(yes.Types, scm.RecordTypeRelease) {
		t.Fatalf("Releases=true must include release type, got %+v", yes.Types)
	}
}
