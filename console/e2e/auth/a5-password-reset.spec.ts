...
test.describe("§6.7.1 A5 · local-cred password reset (console login leg)", () => {
  ...
  test("reset request shows an identical, non-enumerating confirmation for any username", async ({
    page,
  }) => {
    // Existing account.
    ...
    await expect(
      page.getByText(/no such user|user not found|does not exist/i),
    ).toHaveCount(0);
  });

  test("after reset: old session is signed out and the old password is rejected", async ({
    page,
    context,
  }) => {
  ...
  test("next login (A1) succeeds with the new password as the same principal", async ({
    page,
  }) => {
    await requestPasswordReset(page, USER);
    const token = await consumeResetToken(USER);
